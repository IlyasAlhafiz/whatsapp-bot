...
├ _Stiker teks_
├≽️ *.pin <query> [jumlah]*
├ _Gambar dari Pinterest (pinimg)_
╰═══════════════`;
}
...
module.exports = async function responseHandler(text) {
  ...
  // jangan ganggu perintah yang diawali titik
  if (text.trim().startsWith('.')) return null;

  const t = normalize(text);

  // menu & info dinamis
  if (t === 'menu' || t === '.menu' || t === 'help' || t === '.help') {
  ...
  // cocok "contains" – return sekali saja (anti-spam)
  ...
  if (t.includes('siapa yang menciptakanmu')) {
    return 'Saya dikembangkan oleh tim pengembang yang niat bantu kamu. 😊';
  }

  return null; // tidak ada auto-reply => biarkan handler lain
};
