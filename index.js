...
function startTyping(sock, jid) {
  ...
  const tick = async () => {
    ...
    if (!stopped) setTimeout(tick, 6000); // refresh typing
  };
  tick();
...
async function startProgressMessage(sock, jid, initial = '⏳ Sedang diproses…') {
  ...
  async function update(text) {
    try {
      ...
      await sock.sendMessage(jid, { text }); // fallback kalau edit gagal/expired
      return false;
    }
  }
  return { update, key };
...
async function startBot() {
  ...
  sock.ev.on('creds.update', saveCreds);
    ...
    // ⛔ cegah spam (history/echo)
    if (msg.key.fromMe) return;     // jangan respon pesan bot sendiri
    ...
    if (text.startsWith('.tiktok ')) {
      ...
      try {
        ...
        await prog.update('✅ TikTok terkirim.');
      } catch {
        await prog.update('⚠️ Gagal mengunduh TikTok.');
      } finally { stop(); }
      return;
    }
    ...
    if (text.startsWith('.ig ')) {
      ...
      try {
        ...
        await prog.update('⚠️ Gagal mengunduh Instagram.');
      } finally { stop(); }
      return;
    }
    ...
    if (text.startsWith('.yt ')) {
      ...
      try {
        ...
        await prog.update('✅ YouTube terkirim.');
      } catch {
        await prog.update('⚠️ Gagal mengunduh YouTube.');
      } finally { stop(); }
      return;
    }
    ...
    if (text.startsWith('.brat ')) {
      ...
      const stop = startTyping(sock, sender);
      const prog = await startProgressMessage(sock, sender, '⏳ Membuat stiker teks…');
      try {
        await sendTextSticker(sock, sender, stickerText);
        await prog.update('✅ Stiker terkirim.');
      } catch {
        await prog.update('⚠️ Gagal membuat stiker.');
      } finally { stop(); }
      return;
    }
    ...
    if (text.startsWith('.pin ')) {
      ...
      if (/^\d+$/.test(maybeNum)) {
        count = Math.min(Math.max(parseInt(maybeNum, 10), 1), 10);
        query = raw.replace(/\s+\d+$/, '').trim();
      }
      ...
      try {
        ...
        await prog.update(`⚠️ Gagal mengambil gambar untuk *${query}*. Coba kata kunci lain ya.`);
      } finally { stop(); }
      return;
    }

    // commandHandler custom kamu (kalau bukan diawali titik, biarkan responseHandler dulu)
    ...
    // Auto-response (fallback obrolan santai)
