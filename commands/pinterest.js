...
async function ddgImageSearchPinimg(query, max = 3) {
  ...
  while (out.length < max && next) {
    ...
    for (const r of results) { if (out.length >= max) break; out.push({ url: r.image, title: r.title || query, source: r.source || r.url || '' }); }
...
async function searchPinimg(query, count) {
  try { const a = await ddgImageSearchPinimg(query, count); if (a.length) return a; }
...
async function sendPinterestImages(sock, sender, query, count = 3, onProgress) {
  const noop = async () => {}; onProgress = onProgress || noop;
  try {
    await onProgress('search-ddg');
    const results = await searchPinimg(query, count);
    if (!results?.length) { await sock.sendMessage(sender, { text: `❌ Tidak menemukan gambar Pinterest untuk: ${query}` }); return; }
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        ...
        await sock.sendMessage(sender, { image: Buffer.from(imgRes.data), caption: `🖼 Pinterest: ${results[i].title}\n(${i + 1}/${total})` });
        await new Promise(r => setTimeout(r, 600));
      } catch (e) { console.warn('Download image fail:', e.message || e); }
    }
  } catch (e) { await onProgress('search-bing'); throw e; }
}
